...
def build_argue_RNN(lstm_input_dim,
    ...
    sharedFeatures = Input(sharedFeatures_input_dim,
                           name='sharedFeatures')

    lstm1 = LSTM(units_LSTM, return_sequences=False)(sentence1)
    lstm2 = LSTM(units_LSTM, return_sequences=False)(sentence2)
    concatenateLayer = concatenate([lstm1, lstm2, sharedFeatures],
                                   axis=-1)
    dense = Dense(units_Dense, activation='sigmoid')(concatenateLayer)
    softmax = Dense(output_dim, activation='softmax')(dense)
...
def build_FFNN(shared_feature_dim,
    ...
    if has_original_sentences:
        denseOriginal1 = Dense(separate_units,
                               activation=activation)(original1)
        if has_2:
            denseOriginal2 = Dense(separate_units,
                                   activation=activation)(original2)
    for i in range(1, no_separate_layers):
        ...
        if has_2:
            ...
            dense2 = Dense(separate_units,
                           activation=activation)(dense2)
        if has_original_sentences:
            denseOriginal1 = Dropout(rate=dropout_rate)(denseOriginal1)
            denseOriginal1 = Dense(separate_units,
                                   activation=activation)(denseOriginal1)
            if has_2:
                denseOriginal2 = \
    ...
    if has_original_sentences:
        ...
        if has_2:
            ...
            dense2 = Dense(arg_sent_units,
                           activation=activation)(concat2)
        for i in range(1, no_sent_arg_layers):
            arg_sent_units = int(arg_sent_units * layer_decrease_rate)
            dense1 = Dropout(rate=dropout_rate)(dense1)
            dense1 = Dense(arg_sent_units,
                           activation=activation)(dense1)
            if has_2:
                dense2 = Dropout(rate=dropout_rate)(dense2)
                dense2 = Dense(arg_sent_units,
                               activation=activation)(dense2)
    ...
    if has_shared_features:
        if has_original_sentences:
            if has_2:
                model = Model(inputs=[sentence1, sentence2, original1,
                              original2, sharedF], outputs=[softmax])
            else:
                model = Model(inputs=[sentence1, original1, sharedF],
                              outputs=[softmax])
        else:
            if has_2:
                model = Model(inputs=[sentence1, sentence2, sharedF],
                              outputs=[softmax])
            else:
                model = Model(inputs=[sentence1, sharedF],
                              outputs=[softmax])
    else:
        if has_original_sentences:
            if has_2:
                model = Model(inputs=[sentence1, sentence2, original1,
                              original2], outputs=[softmax])
            else:
                model = Model(inputs=[sentence1, original1],
                              outputs=[softmax])
        else:
            if has_2:
                model = Model(inputs=[sentence1, sentence2],
                              outputs=[softmax])
            else:
