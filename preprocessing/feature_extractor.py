...
def add_word_vector_feature(
    ...
    if word2VecModel is None:
        word2VecModel = \
            gensim.models.KeyedVectors.load_word2vec_format(WORD2WEC_EMBEDDING_FILE,
                binary=True)
    ...
    wordVectorFeature = kp_pad_sequences(wordVectorFeature,
            maxlen=pad_no, value=0, padding='post', dtype=float)
...
def add_pos_feature(
    ...
    propositionPOSPadded = kp_pad_sequences(propositionPOSList,
            maxlen=pad_no, value=0, padding='post')
...
def add_keyword_feature(dataset, has_2=True):
    ...
    keywords = dataset[['arg1', 'originalArg1'
            ...
            including_keywords_features(row['arg1'], row['originalArg1'
            ], premise_list, claim_list), axis=1)
    keywords = pd.DataFrame(keywords.tolist(), columns=['arg1',
                            'claimIndicatorArg1', 'premiseIndicatorArg1'
                            ])

    dataset = pd.merge(dataset, keywords, on='arg1')
    if has_2:
        keywords = keywords.rename(columns={'arg1': 'arg2',
...
def including_keywords_features(
    proposition,
    original,
    premise_list,
    claim_list,
...
def add_token_feature(
    ...
    if has_2:
        tokenDataFrame = tokenDataFrame.rename(columns={'arg1': 'arg2',
...
def add_shared_words_feature(
    ...
    if word_type == 'nouns':
        ...
        if word_type == 'verbs':
            ...
            pos_tag_list = []
    if not fullText:
        temp = dataset[[key1, key2]].apply(lambda row: \
                find_shared_words(
                parsedPropositions[propositionSet.index(row[key1])],
                parsedPropositions[propositionSet.index(row[key2])],
        ...
        temp = dataset[[key1, 'fullText1']].apply(lambda row: \
                find_shared_words(
                parsedPropositions[propositionSet.index(row[key1])],
                fullParsedPropositions[fullPropositionSet.index(row['fullText1'
                        ])],
                min_length=min_word_length,
                pos_tag_list=pos_tag_list,
                stemming=stemming,
        ...
        if has_2:
            temp = dataset[[key2, 'fullText1']].apply(lambda row: \
                    find_shared_words(
                    parsedPropositions[propositionSet.index(row[key2])],
                    fullParsedPropositions[fullPropositionSet.index(row['fullText1'
                            ])],
...
def find_shared_words(
    ...
    if stemming == False:
        arg1Nouns = [word for (word, pos) in proposition
                     if (not has_tag_list or pos in pos_tag_list)
...
def add_bert_embeddings(
    ...
    if original_sentence_feature:
        ...
        if has_2:
            emb_frame = \
                emb_frame.rename(columns={'originalArg1': 'originalArg2'
                                 ,
...
def add_sentiment_scores(dataset, key='arg', has_2=True):
    ...
    sid_obj = SentimentIntensityAnalyzer()
    key_t = key.title()
    sentiments = dataset[key + '1'].drop_duplicates().apply(lambda row: \
            sentiment_scores(row, sid_obj))
    sentiments = pd.DataFrame(sentiments.tolist(), columns=[key + '1',
                              'sentNeg' + key_t + '1', 'sentNeu'
                              + key_t + '1', 'sentPos' + key_t + '1',
